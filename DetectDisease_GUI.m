...
axes(handles.axes2);
imshow(I5);title(' Filtered Image ');
handles.ImgData2 = I4;
guidata(hObject,handles);
...
itr = 10;
hWaitBar = waitbar(0,'Evaluating Maximum Accuracy with 200 iterations');
for i = 1:itr
data = Train_Feat;
...
% Visualize Results
if result == 0
    R0 = 'Bacterialleafspot';
    set(handles.edit2,'string',R0);
    set(handles.edit3,'string',Affect);
    helpdlg(' Bacterial leaf spot ');
    disp(' Bacterialleafspot ');
elseif result == 1
    R1 = 'Anthracnose';
...
elseif result == 4
    ...
    helpdlg(' Healthy Betel Leaf ');
    disp('HealthyBetelLeaf ');
end
% Update GUI
guidata(hObject,handles);
